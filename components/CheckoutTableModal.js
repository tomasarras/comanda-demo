...
export default function CheckoutTableModal({ tableId, tableNumber, orders, total, cashierName, onClose, onCheckedOut }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onCheckedOut({ tableId, orderIds: orders.map((o) => o.id) });
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/50 p-0 sm:items-center sm:p-4">
      <div className="w-full max-w-sm rounded-t-2xl bg-white p-5 shadow-xl sm:rounded-2xl">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900">Cobrar mesa {tableNumber}</h2>
          <button type="button" onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100">
            <X size={18} />
          </button>
        </div>
        <p className="mt-1 text-xs text-slate-500">
          {orders.length > 1 ? `${orders.length} órdenes · ` : ""}
          Total: <span className="font-semibold text-slate-700">{formatCurrency(total)}</span>
        </p>
